...
from .config.cooperative_hunting_refuge_config import (
...
def step_world(
    ...
    prey_refuge_escape_probability = cfg["prey_refuge_escape_probability"]
    ...
    active_contribution_tolerance = 1e-12

    if not 0.0 <= prey_refuge_escape_probability <= 1.0:
        raise ValueError("prey_refuge_escape_probability must be within [0, 1]")

    def select_active_hunters(candidate_idxs: List[int]) -> List[int]:
        active_idxs: List[int] = []
        seen = set()
        for idx in candidate_idxs:
            if idx in seen or idx in predators_committed:
                continue
            seen.add(idx)
            if preds[idx].energy * hunt_investment_trait_levels[idx] > active_contribution_tolerance:
                active_idxs.append(idx)
        return active_idxs

    def launch_hunt_attempt(hunter_idxs: List[int]) -> None:
        nonlocal pred_coop_loss
        for idx in hunter_idxs:
            preds[idx].energy, spent = drain_energy(
                preds[idx].energy,
                predator_cooperation_cost_per_unit * hunt_investment_trait_levels[idx],
            )
    ...
    for prey_idx in live_prey_indices:
        ...
        if candidate_pred_idxs:
            if hunt_success_rule == "probabilistic":
                hunter_idxs = select_active_hunters(candidate_pred_idxs)
                if hunter_idxs:
                    ...
                    if pkill > 0.0:
                        launch_hunt_attempt(hunter_idxs)
                        if random.random() >= prey_refuge_escape_probability:
                            kill_success = random.random() < pkill
            elif hunt_success_rule == "threshold_synergy":
                prey_energy = prey.energy
                candidate_hunter_idxs: List[int] = []
                for dy in range(-hunter_pool_radius, hunter_pool_radius + 1):
                    yy = (py + dy) % grid_height
                    for dx in range(-hunter_pool_radius, hunter_pool_radius + 1):
                        xx = (px + dx) % grid_width
                        candidate_hunter_idxs.extend(pred_by_cell.get((xx, yy), []))

                hunter_idxs = select_active_hunters(candidate_hunter_idxs)
                if hunter_idxs:
...
def run_sim(
    ...
    if cfg["enable_live_pygame_renderer"]:
        ...
        live_renderer = PyGameRenderer(
            ...
            auto_fit=True,
            title="Cooperative Hunting Equal-Split Refuge Viewer",
