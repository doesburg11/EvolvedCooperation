#!/usr/bin/env python3
"""Compare runnable mechanism wrappers under matched parameter sweeps.
...
from moran_models.nowak_mechanisms.direct_reciprocity.config.direct_reciprocity_config import config as direct_cfg
from moran_models.nowak_mechanisms.direct_reciprocity.direct_reciprocity_model import run_simulation as run_direct
...
from moran_models.nowak_mechanisms.network_reciprocity.config.network_reciprocity_config import config as net_cfg
from moran_models.nowak_mechanisms.network_reciprocity.network_reciprocity_model import run_simulation as run_network
...
def run_comparison() -> dict[str, str]:
        ...
        for benefit_scale in BENEFIT_SCALES:
            for cost_scale in COST_SCALES:
                for seed in SEEDS:
                    run_i += 1
                    print(
                        f"[compare_runnable_mechanisms] run {run_i}/{total_runs} "
    ...
    rep_path = out_dir / f"compare_runnable_mechanisms_{stamp}_replicates.csv"
    sum_path = out_dir / f"compare_runnable_mechanisms_{stamp}_summary.csv"
    txt_path = out_dir / f"compare_runnable_mechanisms_{stamp}_summary.txt"
    ...
    with txt_path.open("w", encoding="utf-8") as f:
        f.write("Runnable mechanism comparison (matched sweep)\n")
    ...
    print(f"[compare_runnable_mechanisms] wrote replicates -> {rep_path}")
    print(f"[compare_runnable_mechanisms] wrote summary    -> {sum_path}")
    print(f"[compare_runnable_mechanisms] wrote text       -> {txt_path}")
