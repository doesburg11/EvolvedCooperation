#!/usr/bin/env python3
"""Run kin-clustering scaffold proof-of-mechanism checks.

Edit constants in this file, then run from the repo root:

./.conda/bin/python -m moran_models.nowak_mechanisms.direct_reciprocity.scaffolds.kin_clustering.utils.proof_of_mechanism
"""
...
from ..kin_clustering_model import run_simulation


SEEDS = [0, 1, 2, 3, 4]
SIMULATION_STEPS = 500
SUMMARY_INTERVAL_STEPS = 500
OUT_DIR = Path("moran_models/nowak_mechanisms/direct_reciprocity/scaffolds/kin_clustering/data")

SUCCESS_COOPERATION_RATE = 0.60
SUCCESS_RECIPROCAL_FREQUENCY = 0.50
SUCCESS_ALLD_MAX = 0.25

# Scenarios mirror the spatial-clustering scaffold, with one additional ablation
# (no_kin_bias) that tests whether the kin-clustering origin mechanism is
# specifically responsible — the genetic-clustering equivalent of removing the
# grid in the spatial-clustering scaffold.
SCENARIOS: list[tuple[str, dict[str, Any]]] = [
    ...
    (
        "rare_lineage_start",
        {
            "initial_strategy_layout": "reciprocal_lineage",
    ...
    (
        ...
        {
            # kin_interaction_fraction=0.0 → random interaction graph, no kin clustering.
            # Tests whether kin clustering specifically is the origin mechanism,
            # or whether any fixed graph with memory is sufficient.
            "kin_interaction_fraction": 0.0,
        },
    ),
]
...
def main() -> None:
    ...
    replicate_path = OUT_DIR / f"direct_reciprocity_kin_clustering_proof_{stamp}_replicates.csv"
    summary_path = OUT_DIR / f"direct_reciprocity_kin_clustering_proof_{stamp}_summary.csv"
