...
On 2026-04-06, this module directory was renamed from `altruism/` to `ecological_models/spatial_altruism/`.

Stepwise impact:

1. The module now lives at `ecological_models/spatial_altruism/` in the repository.
2. Repo-root run commands should now use `spatial_altruism` package/module names.
3. Analysis scripts that read or write CSV results now point at `ecological_models/spatial_altruism/data/grid_search_results*.csv`.
4. Website GitHub links should now reference `ecological_models/spatial_altruism/` rather than the old folder name.
5. `ecological_models/spatial_altruism/__init__.py` now marks the folder as a Python package so package-aware runners can execute these files as modules.
...
On 2026-04-06, reusable helper code was consolidated into `ecological_models/spatial_altruism/utils/`.

Stepwise impact:

1. `plotly_browser.py` moved to `ecological_models/spatial_altruism/utils/plotly_browser.py`.
2. The duplicated `plot_heatmap_embedded(...)` implementation was extracted into `ecological_models/spatial_altruism/utils/plotting.py`.
3. Pygame helper functions for grid drawing, text drawing, and history plotting were extracted into `ecological_models/spatial_altruism/utils/pygame_helpers.py`.
4. Plotly and heatmap scripts now import these helpers from `ecological_models.spatial_altruism.utils`, and the Pygame UI now does so package-relatively as a module entrypoint.
...
On 2026-04-06, the grid-search runners were moved into `ecological_models/spatial_altruism/utils/`.

Stepwise impact:

1. `altruism_grid_search.py` moved to `ecological_models/spatial_altruism/utils/altruism_grid_search.py`.
2. `altruism_grid_search_original.py` moved to `ecological_models/spatial_altruism/utils/altruism_grid_search_original.py`.
3. `altruism_grid_search_extended_original.py` moved to `ecological_models/spatial_altruism/utils/altruism_grid_search_extended_original.py`.
4. The moved scripts now import `AltruismModel` from the parent package and should be run as modules from the repo root.
5. Result CSV paths remain unchanged within the dataset names, but now read and write `ecological_models/spatial_altruism/data/grid_search_results*.csv`.

## Data Relocation Note

On 2026-04-06, the result CSV files were moved into `ecological_models/spatial_altruism/data/`.

Stepwise impact:

1. `grid_search_results.csv` moved to `ecological_models/spatial_altruism/data/grid_search_results.csv`.
2. `grid_search_results copy.csv` moved to `ecological_models/spatial_altruism/data/grid_search_results copy.csv`.
3. `grid_search_results_extended.csv` moved to `ecological_models/spatial_altruism/data/grid_search_results_extended.csv`.
...
1. `ecological_models/spatial_altruism/requirements.txt` moved to `requirements.txt`.
...
On 2026-04-06, the plotting directory was renamed from `ecological_models/spatial_altruism/plotting/` to `ecological_models/spatial_altruism/images/`.

Stepwise impact:

1. `plot_heatmaps.py` now lives at `ecological_models/spatial_altruism/images/plot_heatmaps.py`.
2. `plot_heatmaps_extended.py` now lives at `ecological_models/spatial_altruism/images/plot_heatmaps_extended.py`.
3. `plot_coexistence_body_plotly.py`, `plot_coexistence_body_plotly_extended.py`, `plot_coexistence_body_plotly_extended copy.py`, and `plot_coexistence_surface_plotly.py` now live in `ecological_models/spatial_altruism/images/`.
4. The generated `plot_coexistence_body_plotly.html` file now lives in `ecological_models/spatial_altruism/images/` so generated visual outputs sit with the image-generation scripts.
...
On 2026-04-06, active runtime defaults were centralized into `ecological_models/spatial_altruism/config/altruism_config.py`.
...
On 2026-04-07, a first culling-only sweep was run and written to `ecological_models/spatial_altruism/data/culling_grid_search_results.csv`.
...
## Usage
### Run the Model
Edit `ecological_models/spatial_altruism/config/altruism_config.py`, then run from the repo root:
```bash
./.conda/bin/python -m ecological_models.spatial_altruism.altruism_model
```

### Run the Pygame UI
Edit `ecological_models/spatial_altruism/config/altruism_config.py` if needed, then run from the repo root:
```bash
...
### Run Grid Search
Edit `ecological_models/spatial_altruism/config/altruism_config.py` and the sweep block in the selected utility, then run from the repo root:
```bash
...
Edit `ecological_models/spatial_altruism/config/altruism_config.py` and the sweep block in the culling utility, then run from the repo root:
```bash
...
After generating `ecological_models/spatial_altruism/data/culling_grid_search_results.csv`, run:
```bash
./.conda/bin/python -m ecological_models.spatial_altruism.images.plot_culling_heatmaps
```
...
```bash
./.conda/bin/python -m ecological_models.spatial_altruism.images.export_culling_summary_heatmaps
```
...
### Import as a Module
```python
from ecological_models.spatial_altruism import AltruismModel, make_params
