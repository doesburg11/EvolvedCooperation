...
def demo_plot(model: AltruismModel, steps: int = 200, interval: float = 0.01):
    if plt is None:
        print("matplotlib not available. Disable demo_plot_enabled in the config or install matplotlib.")
        return
    plt.ion()
    fig, ax = plt.subplots()
...
def main():
    ...
    steps = int(CFG["demo_steps"])

    if bool(CFG["demo_plot_enabled"]):
        demo_plot(
            model,
            steps=steps,
            interval=float(CFG["demo_plot_interval"]),
        )
    else:
