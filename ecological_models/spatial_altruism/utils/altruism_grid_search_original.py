...
`ecological_models/spatial_altruism/config/altruism_config.py`, then run from the repo root:
  ./.conda/bin/python -m ecological_models.spatial_altruism.utils.altruism_grid_search_original
"""
...
def main():
    ...
    if base_params.model_variant != "steady_state":
        raise SystemExit(
            "ecological_models.spatial_altruism.utils.altruism_grid_search_original targets the "
    ...
    found = 0
    n_reps = 10
    csv_path = "ecological_models/spatial_altruism/data/grid_search_results.csv"
    completed = set()
    ...
    print("Results appended to ecological_models/spatial_altruism/data/grid_search_results.csv after each run.")
