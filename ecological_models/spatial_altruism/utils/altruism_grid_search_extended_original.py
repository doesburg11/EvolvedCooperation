#!/usr/bin/env python3
"""
Extended baseline coexistence sweep for the spatial altruism model.
...
`ecological_models/spatial_altruism/config/altruism_config.py`, then run from the repo root:
  ./.conda/bin/python -m ecological_models.spatial_altruism.utils.altruism_grid_search_extended_original
"""
...
if not __package__:
    raise SystemExit(
        ...
        "'./.conda/bin/python -m "
        "ecological_models.spatial_altruism.utils.altruism_grid_search_extended_original'."
    )
...
def main():
    ...
    found = 0
    n_reps = 5
    csv_path = "ecological_models/spatial_altruism/data/grid_search_results_extended.csv"
    completed = set()
    ...
    print("Results appended to ecological_models/spatial_altruism/data/grid_search_results_extended.csv after each run.")
