...
Run from the repo root with:
  ./.conda/bin/python -m ecological_models.cooperative_hunting.utils.tune_mutual_survival
"""
...
def validate_param_grid(param_grid: Dict[str, List[Scalar]]) -> None:
    ...
    for param_name, values in param_grid.items():
        if param_name not in eco.CFG:
            raise ValueError(f"Unknown parameter '{param_name}' in cooperative_hunting.py")
        if not values:
