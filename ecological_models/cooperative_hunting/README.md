...
- `ecological_models/cooperative_hunting/images/` for CSV and text summaries
...
2. Run the exporter from the repository root:
   ...
   ```bash
   ./.conda/bin/python -m ecological_models.cooperative_hunting.utils.export_github_pages_demo
   ```
...
- the browser viewer replays exported states; it does not rerun the Python
  model in JavaScript
- the exporter uses the frozen website config, not the active runtime config
...
- the manifest now records the full replay config and Git commit for stronger
  provenance
...
2. Changed
   [`utils/export_github_pages_demo.py`](./utils/export_github_pages_demo.py)
   so GitHub Pages replay exports use the frozen website config instead of the
   active runtime config.
3. Expanded the replay manifest to record the frozen config source path, the
   full resolved config used for the export, and the Git commit SHA when
   available.
